...
public class ListingDAOImpl implements ListingDAO {
	...
	public void bidOnListing(int listingId, double bid, User user) {
		...
		if (bid > listing.getCurrentPrice()) {
			...
			Logging.Log("info","Your bid is too low! Please enter a price greater than " + listing.getCurrentPrice());
		}
		manager.getTransaction().commit();
	...
	public List<Listing> getListOfListingsForUser(String username) {
		EntityManager manager = factory.createEntityManager();
		TypedQuery<Listing> query = manager
				.createQuery("select listing from Listing listing where listing.user.username = ?", Listing.class);
	...
	public List<Listing> getListOfListingsForUserWinning(String username) {
		EntityManager manager = factory.createEntityManager();
		TypedQuery<Listing> query = manager.createQuery(
				"select listing from Listing listing where listing.winningUser.username = ?", Listing.class);
		query.setParameter(1, username);
		List<Listing> listingList = query.getResultList();
		Logging.Log("info","Get all listings which are currently being won by " + username + ", " + listingList.size()
				+ " listings were found");
		return listingList;
	}
	...
	public List<OldListing> getOldListingsForUser(String username) {
		EntityManager manager = factory.createEntityManager();
		TypedQuery<OldListing> query = manager.createQuery(
				"select listing from OldListing listing where listing.winningUser.username = ?", OldListing.class);
		query.setParameter(1, username);
		List<OldListing> listingList = query.getResultList();
		Logging.Log("info","Get all listings which have been won by " + username + ", " + listingList.size()
				+ " listings were found");
		return listingList;
	}
	...
	public List<OldListing> getOldListingsForSold(String username) {
		EntityManager manager = factory.createEntityManager();
		TypedQuery<OldListing> query = manager.createQuery(
				"select oldListing from OldListing oldListing where oldListing.user.username = ?", OldListing.class);
		query.setParameter(1, username);
		List<OldListing> listingList = query.getResultList();
		Logging.Log("info","Get all listings which have been won by " + username + ", " + listingList.size()
				+ " listings were found");
